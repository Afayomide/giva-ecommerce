...
import { IProduct, ICategory } from "@/types/product.type";
...
export default function ProductDetailsPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Map categories to IDs if they are objects
      const mappedProduct = {
        ...data.data.product,
        categories: data.data.product.categories.map((cat: any) => 
  ...
  const handleToggleCategory = (categoryId: string) => {
    if (!product) return;
    const currentCategories = [...product.categories] as string[];
        ...
        <CardContent className="grid md:grid-cols-2 gap-6">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {availableCategories
                .filter((cat) => (product.categories as string[]).includes(cat._id))
                .map((cat) => (
                  <Badge key={cat._id} variant="default" className="flex items-center gap-1">
                    {cat.name}
                    {editing && (
                      <button
                        type="button"
                ...
                {availableCategories.length > 0 ? (
                  availableCategories.map((cat) => (
                    <Button
                      ...
                      type="button"
                      variant={
                        (product.categories as string[]).includes(cat._id)
                          ? "default"
                          : "outline"
                      }
                      onClick={() => handleToggleCategory(cat._id)}
